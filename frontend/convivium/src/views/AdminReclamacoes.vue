...
.admin-condominio
  .layout-container
    MenuLateral(:itemSelecionado="'reclamacoes'")

    main.area-conteudo
      h2.titulo Administração do Condomínio - Reclamações
...
watch(
  () => route.query.tab,
  (tab) => {
    if (tab === 'moradores' || tab === 'reclamacoes') {
      listaSelecionada.value = tab as string
    } else {
      listaSelecionada.value = 'moradores' // padrão
    }
  },
  { immediate: true },
)
...
/* Área de conteúdo à direita do menu */
.area-conteudo {
  flex: 1;
  padding: 2rem 2.5rem;
  overflow-x: auto;
